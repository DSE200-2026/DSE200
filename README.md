# DSE 200 — Labs

Click a notebook to open it in Google Colab. No sign-in to GitHub is needed.

## Day 1

| Notebook | Session | Open |
| --- | --- | --- |
| **Python Essentials and NumPy** | Chunk 3 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/DSE200-2026/DSE200/blob/main/DSE200_Day1.ipynb) |
| **Data ETL with Pandas** | Chunk 3.5 | [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/DSE200-2026/DSE200/blob/main/DSE200_Day1_ETL.ipynb) |

## Before you start

1. **Save your own copy.** In Colab: *File → Save a copy in Drive*. Otherwise your work is lost when you close the tab.
2. **Run the SETUP cell first.** It downloads `temperatures.csv` from this repository for you.
3. **Lost?** *Runtime → Run all*. Every section rebuilds its own data, so any section works from cold.

## How the cells work

| Cell | What to do |
| --- | --- |
| `▶ DEMO` | Already written. Run it while we talk. |
| `🔍 READ` | **Predict the output before running.** Commit to an answer. |
| `✍️ YOU TRY` | Has a `# TODO`. Yours. |
| `✅ CHECKPOINT` | Run it. It tells you whether you are right. |

> **The course rule:** you must be able to explain every cell you submit.
